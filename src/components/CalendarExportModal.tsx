...
import { generateIcs, downloadIcs, ReminderOption, CalendarExportOptions } from '../utils/calendar'
...
export default function CalendarExportModal({ isOpen, onClose, matches }: CalendarExportModalProps) {
  ...
  const displayedMatches = useMemo(() => {
    ...
    return matches.filter(m => m.HomeTeam === teamFilter || m.AwayTeam === teamFilter)
  }, [matches, teamFilter])
  ...
  const selectStage = (roundNumber: number) => {
    const stageMatches = matches.filter(m => m.RoundNumber === roundNumber).map(m => m.MatchNumber)
  ...
  const handleTeamFilterChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setTeamFilter(team)
    if (team !== 'All Teams') {
      const teamMatches = matches.filter(m => m.HomeTeam === team || m.AwayTeam === team).map(m => m.MatchNumber)
  ...
  const handleExport = async () => {
    setIsExporting(true)
    try {
      const matchesToExport = matches.filter(m => selectedMatches.has(m.MatchNumber))
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate text-left flex items-center gap-2">
                        <CountryFlag team={match.HomeTeam} mr={0} /> {match.HomeTeam} <span className='mx-2 text-xs'>vs</span> <CountryFlag team={match.AwayTeam} mr={0} /> {match.AwayTeam}
                      </div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 flex flex-wrap gap-x-3 mt-1">
                        <span>{new Date(match.DateUtc).toLocaleDateString()}</span>
                        <span>{new Date(match.DateUtc).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} Local</span>
                      </div>
                    </div>
                  </label>
                ))}
            ...
            <section className="space-y-6">
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => setOptions({...options, reminder: parseInt(e.target.value) as ReminderOption})}
                  className="w-full p-2 text-sm border border-gray-300 rounded-md dark:border-gray-600 dark:bg-[#1e1b1b] dark:text-gray-200 focus:ring-2 focus:ring-blue-500"
                >
              ...
              <div>
                ...
                <div className="space-y-3">
                  <label className="flex items-center">
                    ...
                    />
                    <span className="ml-3 text-sm text-gray-700 dark:text-gray-300">Venue / Location</span>
                  </label>
                  <label className="flex items-center">
                    <input 
                      type="checkbox" 
                    ...
                    />
                    <span className="ml-3 text-sm text-gray-700 dark:text-gray-300">Match Details (Stage, Match #, Kickoff)</span>
                  </label>
                  <label className="flex items-center">
                    <input 
                      type="checkbox" 
                      ...
                      onChange={(e) => setOptions({...options, includeTimezone: e.target.checked})}
                    />
                    <span className="ml-3 text-sm text-gray-700 dark:text-gray-300">Timezone Data</span>
                  </label>
                </div>
              </div>
            </section>
